...
export function BookCatalogPage() {
  ...
  const filtered = useMemo(() => data.filter((b) => `${b.title} ${b.author} ${b.genre}`.toLowerCase().includes(search.toLowerCase())), [data, search])
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col gap-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar libros..." className="w-full rounded-md border border-slate-300 bg-slate-50 px-4 py-3 text-sm outline-none" />
      </div>
            ...
            <article key={book.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 dark:border-slate-800 dark:bg-slate-900">
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="flex flex-wrap gap-3">
                  <Link to={`/book/${book.id}`} className="text-sm font-medium text-blue-600 hover:text-blue-700 dark:text-blue-300">Ver detalle</Link>
                  <Button variant="contained" onClick={() => addToCart(book)}>Añadir al carrito</Button>
                </div>
              </div>
            </article>
          ))}
