...
export function BookDetailPage() {
  ...
  if (isLoading || !book) {
    return <div className="rounded-xl bg-white p-10 text-center shadow-sm dark:bg-slate-900">Cargando detalle de libro...</div>
  }
  ...
  return (
    <section className="grid gap-8 lg:grid-cols-[1.3fr_0.7fr]">
      ...
      <aside className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <Link to="/cart" className="block text-center text-sm font-medium text-blue-600 hover:text-blue-700 dark:text-blue-300 dark:hover:text-blue-400">
          Ver carrito
        </Link>
      </aside>
    </section>
